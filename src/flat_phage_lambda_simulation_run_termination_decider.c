...
static BOOL _IsTerminationConditionMet( FLAT_PHAGE_LAMBDA_SIMULATION_RUN_TERMINATION_DECIDER *decider, REACTION *reaction, double time );
static RET_VAL _Destroy( FLAT_PHAGE_LAMBDA_SIMULATION_RUN_TERMINATION_DECIDER *decider );
static RET_VAL _Report( FLAT_PHAGE_LAMBDA_SIMULATION_RUN_TERMINATION_DECIDER *decider, FILE *file );
    
static FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY *
...
static RET_VAL _FreeSpeciesOfInterestArray( FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY **array );

DLLSCOPE SIMULATION_RUN_TERMINATION_DECIDER * STDCALL CreateFlatPhageLambdaSimulationRunTerminationDecider( 
    ...
    FLAT_PHAGE_LAMBDA_SIMULATION_RUN_TERMINATION_DECIDER *decider = NULL;
    
    START_FUNCTION("CreateFlatPhageLambdaSimulationRunTerminationDecider");

    if( ( decider = (FLAT_PHAGE_LAMBDA_SIMULATION_RUN_TERMINATION_DECIDER*)MALLOC( sizeof(FLAT_PHAGE_LAMBDA_SIMULATION_RUN_TERMINATION_DECIDER) ) ) == NULL ) {
        TRACE_0("failed to allocate memory for FLAT_PHAGE_LAMBDA_SIMULATION_RUN_TERMINATION_DECIDER");
        return NULL;
    }
    ...
    if( ( valueString = properties->GetProperty( properties, FLAT_PHAGE_LAMBDA_CI2_THRESHOLD_KEY ) ) == NULL ) {
        decider->cI2Threshold = DEFAULT_FLAT_PHAGE_LAMBDA_CI2_THRESHOLD_VALUE;        
    }
    else {
        if( IS_FAILED( StrToFloat( &(decider->cI2Threshold), valueString ) ) ) {
            decider->cI2Threshold = DEFAULT_FLAT_PHAGE_LAMBDA_CI2_THRESHOLD_VALUE;        
        }
    }
    
    if( ( valueString = properties->GetProperty( properties, FLAT_PHAGE_LAMBDA_CRO2_THRESHOLD_KEY ) ) == NULL ) {
        decider->cro2Threshold = DEFAULT_FLAT_PHAGE_LAMBDA_CRO2_THRESHOLD_VALUE;        
    }
    else {
        if( IS_FAILED( StrToFloat( &(decider->cro2Threshold), valueString ) ) ) {
            decider->cro2Threshold = DEFAULT_FLAT_PHAGE_LAMBDA_CRO2_THRESHOLD_VALUE;        
        }
    }
    
    END_FUNCTION("CreateFlatPhageLambdaSimulationRunTerminationDecider", SUCCESS );
...
static BOOL _IsTerminationConditionMet( FLAT_PHAGE_LAMBDA_SIMULATION_RUN_TERMINATION_DECIDER *decider, REACTION *reaction, double time ) {
    ...
    FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ELEMENT *element = NULL;
    FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ELEMENT **elements = NULL;
    FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY *lysogenySpeciesArray = decider->lysogenySpeciesArray;
    FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY *lysisSpeciesArray = decider->lysisSpeciesArray;
    double cI2Threshold = decider->cI2Threshold;
    double cro2Threshold = decider->cro2Threshold;    
...
static FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY *
_CreateLysogenySpeciesArray( SPECIES **speciesArray, int size ) {
    FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY *array = NULL;
    array = _CreateSpeciesArrayOfInterest( 
                FLAT_PHAGE_LAMBDA_NAME_OF_SPECIES_OF_INTEREST_FOR_LYSOGENY,
                speciesArray,
                size );
...
_CreateLysisSpeciesArray( SPECIES **speciesArray, int size ) {
    FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY *array = NULL;
    array = _CreateSpeciesArrayOfInterest( 
                FLAT_PHAGE_LAMBDA_NAME_OF_SPECIES_OF_INTEREST_FOR_LYSIS,
                speciesArray,
                size );
...
_CreateSpeciesArrayOfInterest( char *nameOfSpeciesOfInterest, SPECIES **speciesArray, int size ) {
    ...
    if( ( element = (FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ELEMENT*)MALLOC( sizeof(FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ELEMENT) ) ) == NULL ) {
    ...
    if( ( elements = (FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ELEMENT**)MALLOC( 
                sizeof(FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ELEMENT*) * listSize ) ) == NULL ) {
        TRACE_1( "elements space for species %s could not be created", nameOfSpeciesOfInterest );
        return NULL;
    }
    ResetCurrentElement( list );    
    while( ( element = (FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ELEMENT*)GetNextFromLinkedList( list ) ) != NULL ) {
        elements[i] = element;
        i++;
    }
    if( ( array = (FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY*)MALLOC( sizeof(FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY) ) ) == NULL ) {
...
static RET_VAL _FreeSpeciesOfInterestArray( FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY **array ) {
    ...
    FLAT_PHAGE_LAMBDA_SPECIES_OF_INTEREST_ARRAY *temp = *array;
