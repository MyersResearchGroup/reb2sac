...
} DIMERIZATION_REDUCTION_INTERNAL2;


static char * _GetDimerizationReductionLevelAssignmentID( ABSTRACTION_METHOD *method );
static RET_VAL _ApplyDimerizationReductionLevelAssignment( ABSTRACTION_METHOD *method, IR *ir );      
static BOOL _IsConditionSatisfied( ABSTRACTION_METHOD *method, REACTION *reaction, DIMERIZATION_REDUCTION_INTERNAL2 *internal ); 
static RET_VAL _DoTransformation( ABSTRACTION_METHOD *method, IR *ir, DIMERIZATION_REDUCTION_INTERNAL2 *internal );
...
static KINETIC_LAW *_CreateReplacementForMonomer( SPECIES *totalSpecies );
static KINETIC_LAW *_CreateReplacementForDimer( SPECIES *totalSpecies );


ABSTRACTION_METHOD *DimerizationReductionLevelAssignmentConstructor(  ABSTRACTION_METHOD_MANAGER *manager ) {
    static ABSTRACTION_METHOD method;
    
    START_FUNCTION("DimerizationReductionLevelAssignmentConstructor");

    if( method.manager == NULL ) {
        method.manager = manager;
        method.GetID = _GetDimerizationReductionLevelAssignmentID;
        method.Apply = _ApplyDimerizationReductionLevelAssignment;
    }
    
    TRACE_0( "DimerizationReductionLevelAssignmentConstructor invoked" );
    
    END_FUNCTION("DimerizationReductionLevelAssignmentConstructor", SUCCESS );
    return &method;
}
...
static char * _GetDimerizationReductionLevelAssignmentID( ABSTRACTION_METHOD *method ) {
    START_FUNCTION("_GetDimerizationReductionLevelAssignmentID");
    
    END_FUNCTION("_GetDimerizationReductionLevelAssignmentID", SUCCESS );
    return "dimerization-reduction-level-assignment";
}
...
static RET_VAL _ApplyDimerizationReductionLevelAssignment( ABSTRACTION_METHOD *method, IR *ir ) {
    ...
    DIMERIZATION_REDUCTION_INTERNAL2 internal;
    
    START_FUNCTION("_ApplyDimerizationReductionLevelAssignment");
    ...
    while( ( reaction = (REACTION*)GetNextFromLinkedList( reactionList ) ) != NULL ) {
        if( _IsConditionSatisfied( method, reaction, &internal ) ) {
            if( IS_FAILED( ( ret = _DoTransformation( method, ir, &internal ) ) ) ) {
                END_FUNCTION("_ApplyDimerizationReductionLevelAssignment", ret );
                return ret;
            }
        }            
    }
            
    END_FUNCTION("_ApplyDimerizationReductionLevelAssignment", SUCCESS );
    return ret;
}      
...
static RET_VAL _DoTransformation( ABSTRACTION_METHOD *method, IR *ir, DIMERIZATION_REDUCTION_INTERNAL2 *internal ) {
    ...
    if( ( monomerReplacement = _CreateReplacementForMonomer( monomer ) ) == NULL ) {        
    ...
    if( ( dimerReplacement = _CreateReplacementForDimer( monomer ) ) == NULL ) {        
        return ErrorReport( FAILING, "_DoTransformation", "could not create a replacement with %s", GetCharArrayOfString( GetSpeciesNodeName( monomer ) ) );
    }
...
static KINETIC_LAW *_CreateReplacementForMonomer( SPECIES *totalSpecies ) {
    ...
    if( ( kineticLaw = CreateRealValueKineticLaw( 0.0 ) ) == NULL ) {
        return NULL;
    }        
...
static KINETIC_LAW *_CreateReplacementForDimer( SPECIES *totalSpecies ) {
    ...
    if( ( kineticLaw = CreateOpKineticLaw( KINETIC_LAW_OP_TIMES, CreateRealValueKineticLaw( 0.5 ), 
          CreateSpeciesKineticLaw( totalSpecies )  ) ) == NULL ) {
        return NULL;
    }        
    return kineticLaw;    
}
