...
static char * _GetStoichiometryAmplificationMethod2ID( ABSTRACTION_METHOD *method );
static RET_VAL _ApplyStoichiometryAmplificationMethod2( ABSTRACTION_METHOD *method, IR *ir );      
...
ABSTRACTION_METHOD *StoichiometryAmplificationMethod2Constructor(  ABSTRACTION_METHOD_MANAGER *manager ) {
    static ABSTRACTION_METHOD method;
    UINT32 amplifier = 0;
    
    START_FUNCTION("StoichiometryAmplificationMethod2Constructor");

    if( method.manager == NULL ) {
        method.manager = manager;
        method.GetID = _GetStoichiometryAmplificationMethod2ID;
        method.Apply = _ApplyStoichiometryAmplificationMethod2;
    }
    
    amplifier = _FindAmplifier( manager );
    method._internal1 = (CADDR_T)amplifier;
    
    TRACE_0( "StoichiometryAmplificationMethod2Constructor invoked" );
    
    END_FUNCTION("StoichiometryAmplificationMethod2Constructor", SUCCESS );
    return &method;
}
...
static char * _GetStoichiometryAmplificationMethod2ID( ABSTRACTION_METHOD *method ) {
    START_FUNCTION("_GetStoichiometryAmplificationMethod2ID");
    
    END_FUNCTION("_GetStoichiometryAmplificationMethod2ID", SUCCESS );
    return "stoichiometry-amplifier2";
}
...
static RET_VAL _ApplyStoichiometryAmplificationMethod2( ABSTRACTION_METHOD *method, IR *ir ) {
    ...
    START_FUNCTION("_ApplyStoichiometryAmplificationMethod2");
    ...
    while( ( reaction = (REACTION*)GetNextFromLinkedList( reactionList ) ) != NULL ) {
        ...
        while( ( edge = GetNextEdge( edges ) ) != NULL ) {
            ...
            if( IS_FAILED( ( ret = ReplaceSpeciesWithKineticLawInKineticLaw( kineticLaw, species, replacement ) ) ) ) {
                END_FUNCTION("_ApplyStoichiometryAmplificationMethod2", ret );
                return ret;
            }        
        ...
        if( ( normalizationTerm = _CreateNormalization( amplifier ) ) == NULL ) {
            return ErrorReport( FAILING, "_ApplyStoichiometryAmplificationMethod2", "could not create normailization term" );
        }
        if( ( kineticLaw = CreateOpKineticLaw( KINETIC_LAW_OP_TIMES, normalizationTerm, kineticLaw ) ) == NULL ) {
            return ErrorReport( FAILING, "_ApplyStoichiometryAmplificationMethod2", "could not create a new kinetic law with normailization term" );
        }   
        SetKineticLawInReactionNode( reaction, kineticLaw ); 
    
    } 
            
    END_FUNCTION("_ApplyStoichiometryAmplificationMethod2", SUCCESS );
    return ret;
}      
...
static KINETIC_LAW *_CreateReactantReplacement( SPECIES *species, UINT32 amplifier, UINT32 stoichiometry ) {
    ...
    replacement = CreateOpKineticLaw( KINETIC_LAW_OP_MINUS, replacement, 
        CreateRealValueKineticLaw( (double)( stoichiometry * amplifier / 4 ) ) );
#endif        
    return replacement;              
}
