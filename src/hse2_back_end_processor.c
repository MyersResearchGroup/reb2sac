...
static RET_VAL _GenerateProductionLogicalStatements( BACK_END_PROCESSOR *backend, REB2SAC_HSE2_PROCESS_INFO *process, int *index, FILE *file );
static RET_VAL _GenerateDegradationLogicalStatements( BACK_END_PROCESSOR *backend, REB2SAC_HSE2_PROCESS_INFO *process, int *index, FILE *file );
...
static RET_VAL _InitProcessInfo( BACK_END_PROCESSOR *backend, SPECIES *target, LINKED_LIST *reactions, REB2SAC_HSE2_PROCESS_INFO *process );
static RET_VAL _CleanProcessInfo( BACK_END_PROCESSOR *backend, REB2SAC_HSE2_PROCESS_INFO *process );
static RET_VAL _GenerateListOfProcesses( BACK_END_PROCESSOR *backend, IR *ir, FILE *file );
static RET_VAL _GenerateProcess( BACK_END_PROCESSOR *backend, REB2SAC_HSE2_PROCESS_INFO *process, FILE *file );

static double _GetInitialConcentration( SPECIES *species );

 
RET_VAL ProcessHse2Backend( BACK_END_PROCESSOR *backend, IR *ir ) {
    RET_VAL ret = SUCCESS;
    char *filename;
    FILE *file = NULL;
    COMPILER_RECORD_T *record = NULL;

    START_FUNCTION("ProcessHse2Backend");
    
    if( !IsTransformableToHse( backend, ir ) ) {
        record = backend->record;
        return ErrorReport( FAILING, "ProcessHse2Backend", "%s cannot be transformed to stochastic asynchronous circuit model", GetCharArrayOfString( record->inputPath ) );
    }   
    ...
    if( ( file = fopen( filename, "w" ) ) == NULL ) {
        return ErrorReport( FAILING, "ProcessHse2Backend", "sbml file open error" ); 
    }
    
    if( IS_FAILED( ( ret = _GenerateCode( backend, ir, file ) ) ) ) {
        END_FUNCTION("ProcessHse2Backend", ret );        
        return ret;
    }
    
    fclose( file );
    
    END_FUNCTION("ProcessHse2Backend", SUCCESS );        
    return ret;
}
...
RET_VAL CloseHse2Backend( BACK_END_PROCESSOR *backend ) {
    RET_VAL ret = SUCCESS;

    START_FUNCTION("CloseHse2Backend");
    
    backend->record = NULL;
        
    END_FUNCTION("CloseHse2Backend", SUCCESS );
...
static RET_VAL _GenerateCode( BACK_END_PROCESSOR *backend, IR *ir, FILE *file ) {
    ...
    if( IS_FAILED( ( ret = _GenerateListOfProcesses( backend, ir, file ) ) ) ) {
        END_FUNCTION("_GenerateCode", ret );        
        return ret;
    }            
    fprintf( file, NEW_LINE );
...
static RET_VAL _GenerateListOfProcesses( BACK_END_PROCESSOR *backend, IR *ir, FILE *file ) {
    ...
    REB2SAC_HSE2_PROCESS_INFO process;
    
    START_FUNCTION("_GenerateListOfProcesses");
    
    list = ir->GetListOfReactionNodes( ir );
    if( ( reactions = CloneLinkedList( list ) ) == NULL ) {
        return ErrorReport( FAILING, "_GenerateListOfProcesses", "could not create a clone of the reaction list" );
    }
    ...
    while( ( target = (SPECIES*)GetNextFromLinkedList( list ) ) != NULL ) {
        if( IS_FAILED( ( ret = _InitProcessInfo( backend, target, reactions, &process ) ) ) ) {
            END_FUNCTION("_GenerateListOfProcesses", ret );        
            return ret;
        }
        if( IS_FAILED( ( ret = _GenerateProcess( backend, &process, file ) ) ) ) {
            END_FUNCTION("_GenerateListOfProcesses", ret );        
            return ret;
        }
        fprintf( file, NEW_LINE );
    ...
    END_FUNCTION("_GenerateListOfProcesses", SUCCESS );        
    return ret;
}
...
static RET_VAL _GenerateProcess( BACK_END_PROCESSOR *backend, REB2SAC_HSE2_PROCESS_INFO *process, FILE *file ) {
    ...
    START_FUNCTION("_GenerateProcess");
    
    fprintf( file, REB2SAC_HSE_START_PROC_FORMAT, GetLogicalSpeciesID( backend, process->target ) );
    fprintf( file, NEW_LINE );
    ...
    fprintf( file, NEW_LINE );
    fprintf( file, REB2SAC_HSE_END_PROC_FORMAT );
    fprintf( file, NEW_LINE );
    
    END_FUNCTION("_GenerateProcess", SUCCESS );        
    return ret;
}
...
static RET_VAL _GenerateDegradationLogicalStatements( BACK_END_PROCESSOR *backend, REB2SAC_HSE2_PROCESS_INFO *process, int *index, FILE *file ) {
    ...
    *index = i;
    ...
    END_FUNCTION("_GenerateLogicalStatements", SUCCESS );        
    return SUCCESS;
}
