...
static KINETIC_LAW *_CreateReplacementForProduct( KINETIC_LAW *con1, KINETIC_LAW *con2 );
static KINETIC_LAW *_CreateReplacementForReactant( KINETIC_LAW *con1, KINETIC_LAW *con2 );
static KINETIC_LAW *_CreateReplacementForModifier( KINETIC_LAW *con1, KINETIC_LAW *con2 );
...
ABSTRACTION_METHOD *NaryOrderUnaryTransformationMethod3Constructor(  ABSTRACTION_METHOD_MANAGER *manager ) {
    static ABSTRACTION_METHOD method;
    HASH_TABLE *table = NULL;
    NARY_ORDER_DECIDER *decider = NULL;
    static CRITICAL_CONCENTRATION_FINDER finder;
    
    START_FUNCTION("NaryOrderUnaryTransformationMethod3Constructor");
    ...
    if( method.manager == NULL ) {
        ...
        if( ( table = CreateHashTable( 64 ) ) == NULL ) {
            END_FUNCTION("NaryOrderUnaryTransformationMethod3Constructor", FAILING );
            return NULL;
        }
    ...
    TRACE_0( "NaryOrderUnaryTransformationMethod3Constructor invoked" );
    
    END_FUNCTION("NaryOrderUnaryTransformationMethod3Constructor", SUCCESS );
    return &method;
}
...
static char * _GetNaryOrderUnaryTransformationMethodID( ABSTRACTION_METHOD *method ) {
    ...
    return "nary-order-unary-transformer3";
}
...
static RET_VAL _HandleReactionM( ABSTRACTION_METHOD *method, IR *ir, IR_EDGE *edge, CRITICAL_CONCENTRATION_INFO *info ) {
    ...
    for( i = 0; i < len; i++ ) {
        ...
        if( i == len2 ) {
            conKineticLaw = _CreateReplacementForModifier( elements[i].conKineticLaw, NULL );
        }
        else {
            conKineticLaw = _CreateReplacementForModifier( elements[i].conKineticLaw, elements[i+1].conKineticLaw );
        }
        if( IS_FAILED( ( ret = ReplaceSpeciesWithKineticLawInKineticLaw( kineticLaw, species, conKineticLaw ) ) ) ) {
            END_FUNCTION("_HandleReactionM", ret );
            return ret;
        }  
        FreeKineticLaw( &conKineticLaw );
        for( j = len2; j >= 0; j-- ) {
    ...
    kineticLaw = GetKineticLawInReactionNode( logicalReaction );
    TRACE_1("logical reaction %s is created", buf );
    conKineticLaw = _CreateReplacementForModifier( NULL, elements[0].conKineticLaw ); 
    if( IS_FAILED( ( ret = ReplaceSpeciesWithKineticLawInKineticLaw( kineticLaw, species, conKineticLaw ) ) ) ) {
        END_FUNCTION("_HandleReactionM", ret );
        return ret;
    }  
...
#else
        if( i == 0 ) {
            conKineticLaw = _CreateReplacementForProduct( NULL, elements[i].conKineticLaw );
            if( IS_FAILED( ( ret = ReplaceSpeciesWithKineticLawInKineticLaw( kineticLaw, species, conKineticLaw ) ) ) ) {
                END_FUNCTION("_HandleReactionP", ret );
                return ret;
            }
            ...
            conKineticLaw = _CreateReplacementForProduct( elements[i-1].conKineticLaw, elements[i].conKineticLaw );
            if( IS_FAILED( ( ret = ReplaceSpeciesWithKineticLawInKineticLaw( kineticLaw, species, conKineticLaw ) ) ) ) {
                END_FUNCTION("_HandleReactionP", ret );
                return ret;
            }
            FreeKineticLaw( &conKineticLaw );  
            if( IS_FAILED( ( ret = _AddNormalizationInSymbolKineticLaw( logicalReaction, elements[i].conKineticLaw, elements[i-1].conKineticLaw, stoichiometry ) ) ) ) {
                END_FUNCTION("_HandleReactionP", ret );
                return ret;
            }
        }
...
#else
        if( i == 0 ) {
            conKineticLaw = _CreateReplacementForReactant( NULL, elements[i].conKineticLaw );
            if( IS_FAILED( ( ret = ReplaceSpeciesWithKineticLawInKineticLaw( kineticLaw, species, conKineticLaw ) ) ) ) {
                END_FUNCTION("_HandleReactionR", ret );
                return ret;
            } 
            ...
            conKineticLaw = _CreateReplacementForReactant( elements[i-1].conKineticLaw, elements[i].conKineticLaw );
            if( IS_FAILED( ( ret = ReplaceSpeciesWithKineticLawInKineticLaw( kineticLaw, species, conKineticLaw ) ) ) ) {
                END_FUNCTION("_HandleReactionR", ret );
                return ret;
            }  
...
static KINETIC_LAW *_CreateReplacementForProduct( KINETIC_LAW *con1, KINETIC_LAW *con2 ) {
    ...
    if( con1 == NULL ) {
        replacement = CloneKineticLaw( con2 );
    }
    else {
        replacement = CreateOpKineticLaw( KINETIC_LAW_OP_PLUS, CloneKineticLaw( con1 ), CloneKineticLaw( con2 ) );
    }    
    replacement = CreateOpKineticLaw( KINETIC_LAW_OP_TIMES, CreateRealValueKineticLaw( 0.5 ), replacement );
    return replacement;
}
...
static KINETIC_LAW *_CreateReplacementForModifier( KINETIC_LAW *con1, KINETIC_LAW *con2 ) {
    KINETIC_LAW *replacement = NULL;
    if( con2 == NULL ) {
        replacement = CloneKineticLaw( con1 );
    }
    else if( con1 == NULL ) {
        replacement = CloneKineticLaw( con2 );
        replacement = CreateOpKineticLaw( KINETIC_LAW_OP_TIMES, CreateRealValueKineticLaw( 0.5 ), replacement );
