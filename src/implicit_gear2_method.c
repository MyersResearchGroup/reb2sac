...
static RET_VAL _PrintStatistics( IMPLICIT_GEAR2_SIMULATION_RECORD *rec, FILE *file);
...
static void SetEventAssignmentsNextValues( EVENT *event, IMPLICIT_GEAR2_SIMULATION_RECORD *rec );
static void SetEventAssignmentsNextValuesTime( EVENT *event, IMPLICIT_GEAR2_SIMULATION_RECORD *rec, double time );
...
DLLSCOPE RET_VAL STDCALL DoImplicitGear2Simulation( BACK_END_PROCESSOR *backend, IR *ir ) {
	...
	return ErrorReport( ret, "DoImplicitGear2Simulation", "Cycle detected in initial and rule assignments" );
      }
