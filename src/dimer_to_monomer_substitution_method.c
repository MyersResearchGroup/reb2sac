...
} DIMER_TO_MONOMER_SUBSTITUTION_INTERNAL;


static char * _GetDimerToMonomerSubstitutionMethodID( ABSTRACTION_METHOD *method );
static RET_VAL _ApplyDimerToMonomerSubstitutionMethod( ABSTRACTION_METHOD *method, IR *ir );      
static BOOL _IsConditionSatisfied( ABSTRACTION_METHOD *method, REACTION *reaction, DIMER_TO_MONOMER_SUBSTITUTION_INTERNAL *internal ); 
static RET_VAL _DoTransformation( ABSTRACTION_METHOD *method, IR *ir, DIMER_TO_MONOMER_SUBSTITUTION_INTERNAL *internal );
...
static KINETIC_LAW *_CreateReplacementForDimer( SPECIES *totalSpecies, KINETIC_LAW *rateRatio );


ABSTRACTION_METHOD *DimerToMonomerSubstitutionMethodConstructor(  ABSTRACTION_METHOD_MANAGER *manager ) {
    static ABSTRACTION_METHOD method;
    
    START_FUNCTION("DimerToMonomerSubstitutionMethodConstructor");

    if( method.manager == NULL ) {
        method.manager = manager;
        method.GetID = _GetDimerToMonomerSubstitutionMethodID;
        method.Apply = _ApplyDimerToMonomerSubstitutionMethod;
    }
    
    TRACE_0( "DimerToMonomerSubstitutionMethodConstructor invoked" );
    
    END_FUNCTION("DimerToMonomerSubstitutionMethodConstructor", SUCCESS );
    return &method;
}
...
static char * _GetDimerToMonomerSubstitutionMethodID( ABSTRACTION_METHOD *method ) {
    START_FUNCTION("_GetDimerToMonomerSubstitutionMethodID");
    
    END_FUNCTION("_GetDimerToMonomerSubstitutionMethodID", SUCCESS );
    return "dimer-to-monomer-substitutor";
}
...
static RET_VAL _ApplyDimerToMonomerSubstitutionMethod( ABSTRACTION_METHOD *method, IR *ir ) {
    ...
    DIMER_TO_MONOMER_SUBSTITUTION_INTERNAL internal;
    
    START_FUNCTION("_ApplyDimerToMonomerSubstitutionMethod");
    ...
    while( ( reaction = (REACTION*)GetNextFromLinkedList( reactionList ) ) != NULL ) {
        if( _IsConditionSatisfied( method, reaction, &internal ) ) {
            if( IS_FAILED( ( ret = _DoTransformation( method, ir, &internal ) ) ) ) {
                END_FUNCTION("_ApplyDimerToMonomerSubstitutionMethod", ret );
                return ret;
            }
        }            
    }
            
    END_FUNCTION("_ApplyDimerToMonomerSubstitutionMethod", SUCCESS );
    return ret;
}      
...
static BOOL _IsConditionSatisfied( ABSTRACTION_METHOD *method, REACTION *reaction, DIMER_TO_MONOMER_SUBSTITUTION_INTERNAL *internal ) {
    ...
    */
    ...
    if( ( internal->rateRatioKineticLaw = CreateRateConstantRatioKineticLaw( kineticLaw ) ) == NULL ) {
        END_FUNCTION("_IsConditionSatisfied", SUCCESS );
        return FALSE;    
    } 
...
static RET_VAL _DoTransformation( ABSTRACTION_METHOD *method, IR *ir, DIMER_TO_MONOMER_SUBSTITUTION_INTERNAL *internal ) {
    ...
    if( ( dimerReplacement = _CreateReplacementForDimer( monomer, rateRatio ) ) == NULL ) {        
    ...
    while( ( edge = GetNextEdge( list ) ) != NULL ) {
        ...
        if( IS_FAILED( ( ret = ir->AddModifierEdge( ir, reaction, monomer, 2 ) ) ) ) {
            END_FUNCTION("_DoTransformation", ret );
            return ret;
        }
#ifdef DEBUG
    ...
    while( ( edge = GetNextEdge( list ) ) != NULL ) {
        ...
        if( IS_FAILED( ( ret = ir->AddReactantEdge( ir, reaction, monomer, 2, NULL, TRUE ) ) ) ) {
            END_FUNCTION("_DoTransformation", ret );
            return ret;
        }        
#ifdef DEBUG
...
static KINETIC_LAW *_CreateReplacementForDimer( SPECIES *monomer, KINETIC_LAW *rateRatio ) {
    KINETIC_LAW *kineticLaw = NULL;
