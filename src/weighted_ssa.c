...
static void SetEventAssignmentsNextValues( EVENT *event, GILLESPIE_MONTE_CARLO_RECORD *rec );
...
static RET_VAL _RunSimulation( GILLESPIE_MONTE_CARLO_RECORD *rec ) {
	...
	if( IS_FAILED( ( ret = _CalculatePredilectionFunctions( rec ) ) ) ) {
	  return ret;
	}
	if( IS_FAILED( ( ret = _CalculateTotalPredilectionFunctions( rec ) ) ) ) {
	  return ret;
	}
...
static RET_VAL _CalculateTotalPredilectionFunctions( GILLESPIE_MONTE_CARLO_RECORD *rec ) {
...
static RET_VAL _CalculatePredilectionFunctions( GILLESPIE_MONTE_CARLO_RECORD *rec ) {
  // loop through all reactions (see _CalculatePropensities)
}


static RET_VAL _CalculatePredilectionFunction( GILLESPIE_MONTE_CARLO_RECORD *rec, REACTION *reaction ) {
