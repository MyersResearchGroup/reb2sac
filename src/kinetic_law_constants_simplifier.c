...
RET_VAL SimplifyInitialAssignment( KINETIC_LAW *kineticLaw ) {
    ...
    if( kineticLaw == NULL || IS_FAILED( ( ret = kineticLaw->Accept( kineticLaw, &visitor ) ) ) ) {    
...
static RET_VAL _FindChildValue(KINETIC_LAW_VISITOR *visitor, LINKED_LIST *children, UINT i, double *childValue) {
    RET_VAL ret = SUCCESS;
...
static RET_VAL _VisitPWToSimplifyInitial( KINETIC_LAW_VISITOR *visitor, KINETIC_LAW *kineticLaw ) {
	...
	result = result > childValue ? childValue : result;
      }
      break;
    ...
    case KINETIC_LAW_OP_LT:
      result = 1;
      for ( i = 1; i < num; i++ ) {
	if ((ret=_FindChildValue(visitor, children, i-1, &childValue))!=CHANGE) {
	  return ret;
	} else {
	  ret = SUCCESS;
	}
	if ((ret=_FindChildValue(visitor, children, i, &childValue2))!=CHANGE) {
	  return ret;
	} else {
	  ret = SUCCESS;
	}
...
static RET_VAL _VisitOpToSimplifyInitial( KINETIC_LAW_VISITOR *visitor, KINETIC_LAW *kineticLaw ) {
    ...
    switch( GetOpTypeFromKineticLaw( kineticLaw ) ) {
	  ...
	  result = floor(leftValue / rightValue);
        break;
...
static RET_VAL _VisitUnaryOpToSimplifyInitial( KINETIC_LAW_VISITOR *visitor, KINETIC_LAW *kineticLaw ) {
    ...
    if ( GetUnaryOpTypeFromKineticLaw( kineticLaw ) != KINETIC_LAW_UNARY_OP_RATE) {
      if( IS_FAILED( ( ret = child->Accept( child, visitor ) ) ) ) {
	...
	childValue = GetCurrentRateInSymbol( GetSymbolFromKineticLaw( child ) );
      } else if (IsSpeciesKineticLaw( child )) {
	childValue = GetRateInSpeciesNode( GetSpeciesFromKineticLaw( child ) );
      } else if (IsCompartmentKineticLaw( child )) {
	childValue = GetCurrentRateInCompartment( GetCompartmentFromKineticLaw( child ) );
      } else {
	END_FUNCTION("_VisitUnaryOpToEvaluate", E_WRONGDATA );
	return ret;
      }
    }    
    else if( IsRealValueKineticLaw( child ) ) {
        childValue = GetRealValueFromKineticLaw( child );
    }
    else if( IsSymbolKineticLaw( child ) ) {
#if 1
