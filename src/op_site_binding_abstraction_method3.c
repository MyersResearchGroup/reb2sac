...
#if 0
typedef struct {
    LINKED_LIST *transcriptionFactors;
    SPECIES *complex;
    SPECIES *product;    
    LINKED_LIST *transcriptionFactorEdges;
    IR_EDGE *productEdge;    
    KINETIC_LAW *k1RatioKineticLaw;
    KINETIC_LAW *k2KineticLaw;
    REACTION *complexFormationReaction;
    REACTION *releaseReaction;
} OP_SITE_REDUCTION_3_INTERNAL_ELEMENT;
 
typedef struct {
    SPECIES *operator;
    LINKED_LIST *elements;
} OP_SITE_REDUCTION_3_INTERNAL;
#endif
...
ABSTRACTION_METHOD *OpSiteBindingAbstractionMethod3Constructor(  ABSTRACTION_METHOD_MANAGER *manager ) {
...
static char * _GetOpSiteBindingMethodID( ABSTRACTION_METHOD *method ) {
    ...
    return "operator-site-reduction-1";
}
...
static RET_VAL _DoTransformation( ABSTRACTION_METHOD *method, IR *ir, SPECIES *op ) {
    ...
    if( ( massActionTable = CreateHashTable( 128 ) ) == NULL ) {
    ...
    if( ( products = CreateLinkedList() ) == NULL ) {
    ...
    while( ( edge = GetNextEdge( edges ) ) != NULL ) {
        ...
        while( ( modifierEdge = GetNextEdge( modifierEdges ) ) != NULL ) {     
            ...
            while( ( productEdge = GetNextEdge( productEdges ) ) != NULL ) {
                ...
                if( IS_FAILED( ( ret = _AddSpeciesInList( product, products ) ) ) ) {
                    return ErrorReport( FAILING, "_DoTransformation", 
                                        "could not %s to product list", 
                                        GetCharArrayOfString( GetSpeciesNodeName( product ) ) );                    
                }
            }
        }        
    }
...
#ifdef DEBUG
    printf( "products transcribed from operator %s is:" NEW_LINE, 
            GetCharArrayOfString( GetSpeciesNodeName( op ) ) );
    ResetCurrentElement( products );
    while( ( product = (SPECIES*)GetNextFromLinkedList( products ) ) != NULL ) {
        printf( "\t%s" NEW_LINE, GetCharArrayOfString( GetSpeciesNodeName( product ) ) );        
    }
#endif
