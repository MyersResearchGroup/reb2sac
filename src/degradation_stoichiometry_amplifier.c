...
ABSTRACTION_METHOD *DegradationStoichiometryAmplificationMethodConstructor(  ABSTRACTION_METHOD_MANAGER *manager ) {
    static ABSTRACTION_METHOD method;
    UINT32 amplifier = 0;
    
    START_FUNCTION("DegradationStoichiometryAmplificationMethodConstructor");

    if( method.manager == NULL ) {
        method.manager = manager;
        method.GetID = _GetDegradationStoichiometryAmplificationMethodID;
        method.Apply = _ApplyDegradationStoichiometryAmplificationMethod;
    }
    
    amplifier = _FindAmplifier( manager );
    method._internal1 = (CADDR_T)amplifier;
    
    TRACE_0( "DegradationStoichiometryAmplificationMethodConstructor invoked" );
    
    END_FUNCTION("DegradationStoichiometryAmplificationMethodConstructor", SUCCESS );
    return &method;
}
...
static char * _GetDegradationStoichiometryAmplificationMethodID( ABSTRACTION_METHOD *method ) {
    START_FUNCTION("_GetDegradationStoichiometryAmplificationMethodID");
    
    END_FUNCTION("_GetDegradationStoichiometryAmplificationMethodID", SUCCESS );
    return "degradation-stoichiometry-amplifier";
}
...
static RET_VAL _ApplyDegradationStoichiometryAmplificationMethod( ABSTRACTION_METHOD *method, IR *ir ) {
    ...
    START_FUNCTION("_ApplyDegradationStoichiometryAmplificationMethod");
    ...
    while( ( reaction = (REACTION*)GetNextFromLinkedList( reactionList ) ) != NULL ) {
        ...
        while( ( edge = GetNextEdge( edges ) ) != NULL ) {
            ...
            if( IS_FAILED( ( ret = ReplaceSpeciesWithKineticLawInKineticLaw( kineticLaw, species, replacement ) ) ) ) {
                END_FUNCTION("_ApplyDegradationStoichiometryAmplificationMethod", ret );
                return ret;
            }        
        ...
        if( ( normalizationTerm = _CreateNormalization( amplifier ) ) == NULL ) {
            return ErrorReport( FAILING, "_ApplyDegradationStoichiometryAmplificationMethod", "could not create normailization term" );
        }
        if( ( kineticLaw = CreateOpKineticLaw( KINETIC_LAW_OP_TIMES, normalizationTerm, kineticLaw ) ) == NULL ) {
            return ErrorReport( FAILING, "_ApplyDegradationStoichiometryAmplificationMethod", "could not create a new kinetic law with normailization term" );
        }   
        SetKineticLawInReactionNode( reaction, kineticLaw ); 
    
    } 
            
    END_FUNCTION("_ApplyDegradationStoichiometryAmplificationMethod", SUCCESS );
    return ret;
}      
